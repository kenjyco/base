...
do_install() {
    if [[ -f /usr/bin/apt-get ]]; then
        ...
        if [[ -n "$extras" ]]; then
            ...
            sudo apt-get install -y vim zsh zsh-syntax-highlighting fish tmux htop glances jq pmount acpi dkms openssh-server colordiff tree ncdu ranger fzf pv nnn w3m w3m-img nmap mtr tldr sqlite3 imagemagick pandoc lynx exuberant-ctags || return 1
        ...
        if [[ -n "$extras" ]]; then
            echo -e "\nInstalling/upgrading other useful CLI packages..."
            sudo yum install -y vim-enhanced zsh zsh-syntax-highlighting fish tmux htop glances jq pmount acpi dkms openssh-server colordiff tree ncdu ranger fzf pv nnn w3m w3m-img nmap mtr tldr ImageMagick pandoc lynx ctags || return 1
